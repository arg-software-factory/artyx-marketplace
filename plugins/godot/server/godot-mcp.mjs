...
// node_modules/@coding-solo/godot-mcp/build/index.js
...
var GODOT_DEBUG_MODE = true;
var execFileAsync = promisify(execFile);
var __filename = fileURLToPath(import.meta.url);
var __dirname = dirname(__filename);
var GodotServer = class {
  server;
  activeProcess = null;
  godotPath = null;
  operationsScriptPath;
  validatedPaths = /* @__PURE__ */ new Map();
  ...
  constructor(config) {
    ...
    if (config) {
      ...
      if (config.godotDebugMode !== void 0) {
        godotDebugMode = config.godotDebugMode;
      }
      if (config.strictPathValidation !== void 0) {
  ...
  async detectGodotPath() {
    ...
    const possiblePaths = [
      "godot"
      // Check if 'godot' is in PATH first
    ];
  ...
  async executeOperation(operation, params, projectPath) {
    ...
    try {
      ...
      const args = [
        ...
        "--path",
        projectPath,
        // Safe: passed as argument, not interpolated into shell command
        "--script",
        this.operationsScriptPath,
        operation,
        paramsJson
        // Safe: passed as argument, not interpreted by shell
      ];
      if (GODOT_DEBUG_MODE) {
    ...
    try {
      ...
      for (const entry of entries) {
        if (entry.isDirectory()) {
          ...
          } else if (dirName === "assets" || dirName === "textures" || dirName === "models" || dirName === "sounds" || dirName === "music") {
      ...
      if (!recursive) {
        ...
        for (const entry of entries) {
          if (entry.isDirectory()) {
            ...
            const projectFile2 = join(subdir, "project.godot");
            if (existsSync(projectFile2)) {
              projects.push({
                path: subdir,
                name: entry.name
              });
  ...
  async handleLaunchEditor(args) {
    ...
    try {
      ...
      const process3 = spawn(this.godotPath, ["-e", "--path", args.projectPath], {
        stdio: "pipe"
      });
  ...
  async handleRunProject(args) {
    ...
    try {
      ...
      const process3 = spawn(this.godotPath, cmdArgs, { stdio: "pipe" });
      const output = [];
      const errors = [];
      ...
      process3.on("exit", (code) => {
        ...
        if (this.activeProcess && this.activeProcess.process === process3) {
          this.activeProcess = null;
        }
      });
      process3.on("error", (err) => {
      ...
      this.activeProcess = { process: process3, output, errors };
      return {
        content: [
          {
            type: "text",
...
Output: ${stdout}`
  ...
  async handleLoadSprite(args) {
    ...
    if (!this.validatePath(args.projectPath) || !this.validatePath(args.scenePath) || !this.validatePath(args.nodePath) || !this.validatePath(args.texturePath)) {
      return this.createErrorResponse("Invalid path", ['Provide valid paths without ".." or other potentially unsafe characters']);
    }
    try {
      const projectFile = join(args.projectPath, "project.godot");
...
Output: ${stdout}`
  ...
  async run() {
    try {
      ...
      const isValid2 = await this.isValidGodotPath(this.godotPath);
      if (!isValid2) {
        if (this.strictPathValidation) {
